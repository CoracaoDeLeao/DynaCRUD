...
public class Modal_Create_Record_Controller implements Initializable{
    ...
    public void setConstructor(Connection conn, Stage modal, String schema, String table, Runnable onCloseCallback) {
        this.conexao = conn;
    ...
    @FXML
    private void createRecord(ActionEvent event) {
        RecordDAO_Imp record = new RecordDAO_Imp();
        // Chamando o método assíncrono e aguardando a execução
        record.createRecord(conexao, schema, table, fields)
            .thenRun(() -> {
	            Platform.runLater(() -> {
